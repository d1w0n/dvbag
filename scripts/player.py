...
class Player(Instance):
    ...
    def update_health_ui(self, ui_list):
        for element in ui_list:
            if element.name == "HealthBar": element.stat = self.health
            elif element.name == "HealthText": element.set_text("Health: " + str(self.health) + "")
    ...
    def update(self, data): 
        ...
        if not self.invulnerable and (pygame.time.get_ticks() - self.damage_ticks) > self.damage_cooldown:
            for enemy in self.nearby["Enemy"]:
                if enemy.type not in ("Enemy", "ChargerEnemy"): continue
                if self.get_collision(enemy): self.recieve_damage(enemy.damage, data)

            for projectile in self.nearby["Projectile"]:
                if not projectile.type == "EnemyProjectile": continue
                if self.get_collision(projectile): self.recieve_damage(projectile.damage, data)
        ...
        if key[pygame.K_w]: self._dy = -self.speed
        if key[pygame.K_a]: self._dx = -self.speed
        if key[pygame.K_s]: self._dy = self.speed
        if key[pygame.K_d]: self._dx = self.speed
        # player movement.

        if self.health <= 0 and not config.IMMORTAL: self.remove = True
    ...
    def tick(self, data):
        ...
        if not self.invulnerable:
            ...
            if self.weapon == "Projectile":
                ...
                if pygame.mouse.get_pressed()[2] and (pygame.time.get_ticks() - self.secondary_ticks) > self.secondary_cooldown:
                    ...
                    for _ in range(3): data["instances"].add_ProjectileParticle(self._window, "assets/images/magentadot.png", self.x, self.y, 12, 12, math.degrees(math.atan2(mouse_y + data["camera"].y - self.y, mouse_x + data["camera"].x - self.x)) + random.randint(-45, 45), 20, 1, 150)
                ...
                if pygame.mouse.get_pressed()[0] and (pygame.time.get_ticks() - self.primary_ticks) > self.primary_cooldown:
                    self.primary_ticks = pygame.time.get_ticks()
                    for _ in range(5): data["instances"].add_Beam(self._window, self.x, self.y, 24, 24, math.degrees(math.atan2(mouse_y + data["camera"].y - self.y, mouse_x + data["camera"].x - self.x)) + random.randint(-30, 30), 15)
                    for _ in range(3): data["instances"].add_ProjectileParticle(self._window, "assets/images/magentadot.png", self.x, self.y, 12, 12, math.degrees(math.atan2(mouse_y + data["camera"].y - self.y, mouse_x + data["camera"].x - self.x)) + random.randint(-45, 45), 20, 1, 150)
                    data["camera"].add_shake(5)
        ...
        if self.dashing: data["instances"].add_AfterImage(self._window, self.spritepath, self.x, self.y, self.width, self.height, self._angle, 250, 125, -1)
